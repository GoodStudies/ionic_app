// until now the study id is hardcoded
export const get_fixed = "https://api.goodstudies.de/questions/study/2/fixed";
export const get_groups = "https://api.goodstudies.de/question_groups";
export const get_subgroups_id =
  "https://api.goodstudies.de/question_sub_groups";
export const get_question_id = "https://api.goodstudies.de/questions/";
export const get_questions = "https://api.goodstudies.de/questions";
export const get_subgroups = "https://api.goodstudies.de/question_sub_groups";
export const loginRequest = "https://api.goodstudies.de/login";
